...
void Population::gerer(int tempsEcoule, vector<Ascenseur>& listes, LoaderObject& objects)
{
    ...
    vector<size_t> listes_indexAscenseursNonHave;
    vector<int> c;
    float ta;
    bool find=false;
    int best_index=-1;
...
void gererPersonnesAscenseur(Ascenseur& ascenseur, Population& pop, LoaderObject& objects)
{
    SDL_Rect p;
    ...
    for (size_t i=0;i<ascenseur.getPersonnes().size();i++)
    {
        ...
        if (pop.getListe()[ascenseur.getPersonnes()[i]].getNextEtages()==(int)ascenseur.getCurrentEtages())
        {
            ...
            pop.getListe()[ascenseur.getPersonnes()[i]].setColsAim(-1);
            ...
            pop.getListe()[ascenseur.getPersonnes()[i]].setAimEtage(0);
