...
int main (int argc, char** argv)
{
    ...
    cout << "Entrez la vitesse de la simulation." << endl;
    cin >> SPEED_SIMULATION;
    srand(time(NULL));
    ...
    int tempsActuel = 0, tempsPrecedent = 0, tempsDebutProgramme = SDL_GetTicks();
    int cameraVx = 0, cameraVy = 0;
    int ongletAscenseur = 0, ongletPopulation = 0;
    SDL_Rect camera={0};
    ...
    sInfos.isHoverPageN=false; sInfos.isHoverPageP=false, sInfos.isHoverPersInAsc=false;
    ...
    while (continuer)
    {
            ...
            switch (event.type)
            {
                ...
                case SDL_KEYDOWN:
                    switch(event.key.keysym.sym)
                    {
                        ...
                        break;
                        case SDLK_RIGHT:
                            if (ongletAscenseur<ascenseurs.size()-1)
                                ongletAscenseur++;
                            break;
                        case SDLK_LEFT:
                            if (ongletAscenseur>0)
                                ongletAscenseur--;
                            break;
                        case SDLK_UP:
                            if (ongletPopulation*5+5<NOMBRE_PERSONNES-1)
                                ongletPopulation++;
                            break;
                        case SDLK_DOWN:
                            if (ongletPopulation>0)
                                ongletPopulation--;
                            break;
                    }
                break;
        ...
        tempsActuel=SDL_GetTicks()*SPEED_SIMULATION; /// Renvoie le nombre de ms depuis le début du programme
            if (!pause)
            {
                ...
                pop.gerer(tempsActuel-tempsPrecedent, ascenseurs, surfaces); /// Donnes des "ordres" à la population, et aux ascenseurs
                bougerAscenseurs(surfaces, ascenseurs, tempsActuel-tempsPrecedent, pop); /// Moteur physique des ascenseurs
                bougerPersonnes(surfaces, pop, tempsActuel-tempsPrecedent, ascenseurs); /// Moteur physique des personnes
            }
            else
            {
                tempsDebutProgramme+=(tempsActuel-tempsPrecedent);
            }
        ...
        SDL_FillRect(surfaces.getSurface(ECRAN), 0, SDL_MapRGB(surfaces.getSurface(ECRAN)->format, 0, 0, 0));
            ...
            afficherPersonnes(surfaces, pop, camera, sInfos); /// La gui du programme
            afficherTooltip(surfaces, pop, ascenseurs, texte, police, sInfos); /// La gui du programme
            ecrireHeure(surfaces, tempsActuel, tempsDebutProgramme, texte, police); /// La gui du programme
