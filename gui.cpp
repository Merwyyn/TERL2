...
void afficherAscenseurs(LoaderObject& objects, vector<Ascenseur>& ascenseurs, SDL_Rect camera, sourisTooltip& sInfos)
{
    ...
    for (size_t j=0;j<ascenseurs.size();j++)
    {
        pos.x=j*objects.getSurface(ASCENSEUR)->w+25-camera.x;
...
void ecrireHeure(LoaderObject& objects, int tempsActuel, int tempsDebutProgramme, SDL_Surface *texte, TTF_Font *police)
{
    SDL_Color couleurNoire = {0, 0, 0};
    SDL_Rect pos;
    pos.x=20;
    pos.y=objects.getSurface(ECRAN)->h-20;
    int timestamp=(tempsActuel-tempsDebutProgramme)/1000;
    int h=7,m=59,s=30;
    h+=timestamp/3600;
    timestamp-=(h-7)*3600;
    m+=timestamp/60;
...
void afficherPersonnes(LoaderObject& objects, Population& population, SDL_Rect camera, sourisTooltip& sInfos)
{
    SDL_Rect pos;
