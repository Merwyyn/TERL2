...
using namespace std;
void bougerAscenseurs(LoaderObject& objects, vector<Ascenseur>& ascenseurs, int tempsEcoule, Population& population)
{
    float mouvementPx=0;
    long vitesse=0;
    int currentPos=0;
    ...
    for (size_t i=0;i<ascenseurs.size();i++)
    {
        ...
        if (vitesse!=0) /// L'ascenseur est en mouvement
        {
            ...
            if ((vitesse>0 && currentPos+mouvementPx>=aimPx) || (vitesse<0 && currentPos+mouvementPx<=aimPx)) /// S'il est arrivé à destination (ou un peu plus loin)
            {
                ...
                gererPersonnesAscenseur(ascenseurs[i], population, objects); /// Et on gère les personnes qui sont à l'intérieur
            }
            ascenseurs[i].setPosition(currentPos+mouvementPx);
        }
        else
        {
            gererPersonnesAscenseur(ascenseurs[i], population, objects);
        }
    }
}
void bougerPersonnes(LoaderObject& objects, Population& population, int tempsEcoule, vector<Ascenseur>& ascenseurs)
{
    float mouvementPx=0;
    long vitesse=0;
    ...
    for (size_t i=0;i<NOMBRE_PERSONNES;i++)
    {
        ...
        if (population.getListe()[i].getVisible() && vitesse!=0) /// Si on est visible et qu'on bouge
        {
            ...
            if ((vitesse<0 && currentPosX+mouvementPx<=aimPx) || (vitesse>0 && currentPosX+mouvementPx>=aimPx)) /// On est arrivé à destination
            {
                ...
                if (vitesse<0 && population.getListe()[i].getColsAim()!=0) /// on sort de l'écran
                {
